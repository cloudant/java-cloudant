...
public class CouchDbDesign {
    ...
    public List<DesignDocument> getAllFromDesk() {
        ...
        for (String docName : CouchDbUtil.listResources(format("%s/", DESIGN_DOCS_DIR))) {
    ...
    public DesignDocument getFromDesk(String id) {
        ...
        final List<String> elements = CouchDbUtil.listResources(rootPath);
        if (elements == null) {
        ...
        if (elements.contains(VIEWS)) {
            ...
            for (String viewDirName : CouchDbUtil.listResources(viewsPath)) { // views sub-dirs
                final DesignDocument.MapReduce mr = new DesignDocument.MapReduce();
                final String viewPath = format("%s%s/", viewsPath, viewDirName);
                final List<String> dirList = CouchDbUtil.listResources(viewPath);
                for (String fileName : dirList) { // view files
                    final String def = CouchDbUtil.readFile(format("/%s%s", viewPath, fileName));
        ...
        dd.setRewrites(dbc.getGson().fromJson(readContent(elements, rootPath, REWRITES),
                JsonArray.class));
        dd.setFulltext(dbc.getGson().fromJson(readContent(elements, rootPath, FULLTEXT),
                JsonObject.class));
        dd.setIndexes(dbc.getGson().fromJson(readContent(elements, rootPath, INDEXES), JsonObject
                .class));
        return dd;
    }

    private Map<String, String> populateMap(String rootPath, List<String> elements, String
            element) {
        ...
        if (elements.contains(element)) {
            ...
            for (String fileName : CouchDbUtil.listResources(path)) {
                String contents = CouchDbUtil.readFile(format("/%s%s", path, fileName));
    ...
    public String readContent(List<String> elements, String rootPath, String element) {
        if (elements.contains(element)) {
            String path = format("%s%s/", rootPath, element);
            List<String> dirList = CouchDbUtil.listResources(path);
            for (String file : dirList) {
                String contents = CouchDbUtil.readFile(format("/%s%s", path, file));
                return contents;
            }
        }
        return null;
